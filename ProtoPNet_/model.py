...
from base_model import base_model_features
from base_model import My_CNN
# from receptive_field import compute_proto_layer_rf_info_v2

base_architecture_to_features = {'base_feature': base_model_features}
...
class PPNet(nn.Module):
    ...
    def __init__(self, features, sequence_length, prototype_shape,
        ...
        #assert latent_weight >= 0 and latent_weight <= 1, "Error: Latent weight must be in [0, 1]"
        #self.latent_weight = latent_weight
        #print("Latent weight: {}".format(latent_weight))
        ...
        first_add_on_layer_in_channels = [i for i in features.modules() if isinstance(i, nn.Conv1d)][-1].out_channels
    ...
    def prototype_distances(self, x):
        ...
        latent_distances = self.cosine_similarity(conv_features, self.prototype_vectors[:, :-1])
        input_distances = self.cosine_similarity(avg_pooled_x, self.prototype_vectors[:, -1:])
...
def construct_PPNet(base_architecture, pretrained=True, sequence_length=887,
                    prototype_shape=(360, 128, 1, 1), num_classes=10,
                    prototype_activation_function='log',
